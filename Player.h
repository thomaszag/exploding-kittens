...
public:
	...
	// Postcondition: A player with a name, an empty hand, and m_lost set to fals\
		e
	Player(string name, vector<Card> hand, bool lost);

	// Name - PlayCard()
	// Desc - Function for moving a card from a player's hand to the discard pile
	// Preconditions - A hand with at least 1 card
	// Postconditions - Returns a card from the hand and deletes it from the arra\
		y
	Card PlayCard(int choice);
